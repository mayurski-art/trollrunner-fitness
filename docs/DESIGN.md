...
| 14 | Education + polish | **DONE — all 15 phases shipped.** `/learn` hub with 8 evidence-informed articles (Next.js SSG); PWA installable (`manifest.ts`, generated favicon/apple-icon/192/512 icons via `next/og` `ImageResponse`, no external image assets); a11y: skip-to-content link, `#main-content` landmark, disclosure `aria-expanded`/`aria-controls` on the coach chat; SEO/OG: `opengraph-image.tsx`, `robots.ts`, `sitemap.ts`, full OG/Twitter metadata on the root layout. |
